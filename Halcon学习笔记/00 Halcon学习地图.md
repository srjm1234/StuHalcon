...
> [!tip] 最后一步：把检测结论落成 OK/NG
> 前面所有主线解决的是"测出多少"，最后还要回答"合不合格"。
> [[17 综合实战 五个作业题与判定逻辑]] 用五份作业串起全部主线，并把重点放在两件事上：
> **① 判定逻辑**（计数 / 尺寸 / 面积 / 交叉验证各自怎么写、**为什么不能对浮点用 `==`**）；
> **② 逐份挑出源码里真实存在的 bug** —— 其中"**循环里忘了把 `Image` 换成 `Image1`**"这一个错误在三份源码里重复出现，
> 现象是"**图在变、结果不变**"，极具迷惑性。
...
| 17 | [[17 综合实战 五个作业题与判定逻辑]] | 五份作业串起全部主线；OK/NG 判定怎么写；**逐份源码纠错** | `综合练习/*.hdev` |
...
- [ ] 判定的四种判据（计数 / 尺寸 / 面积 / 交叉验证）分别适合什么场景？
- [ ] 为什么判定不能对浮点用 `==`？容差的量级该怎么定？
- [ ] "图在变、结果不变"最可能是什么原因？怎么排查？
- [ ] 为什么基准图与待测图的预处理参数必须完全一致？
